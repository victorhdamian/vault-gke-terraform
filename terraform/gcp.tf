...
/*
# bucket vault-xxxxxxxxxxx-vault-terraform-storage most exist
# interpolation is not allow for backends so list the values
terraform {
  backend "gcs" {
    bucket      = "vault-xxxxxxxxxxx-vault-terraform-storage"
    prefix      = "terraform/state"
    project     = "vault-xxxxxxxxxxx"
  }
}
*/
...
# Create the vault-terraform storage bucket backend
resource "google_storage_bucket" "vault-terraform" {
  name          = "${local.vault_project_id}-vault-terraform-storage"
  project       = "${local.vault_project_id}"
  force_destroy = true
  storage_class = "MULTI_REGIONAL"
...
/*
# Create vault backend storage backup bucket
resource "google_storage_bucket" "vault-terraform" {
  name          = "${google_project.vault-staging-gl.project_id}-vault-terraform-storage"
  project       = "${google_project.vault-staging-gl.project_id}"
  force_destroy = false
  storage_class = "MULTI_REGIONAL"
...
GS backup
requires:
enable service: storagetransfer.googleapis.com
permisions granted to storage management service account:
*/
/*
# Create vault backend storage backup bucket
resource "google_storage_bucket" "vault_backup" {
  name          = "${google_project.vault-staging-gl.project_id}-vault-backup-storage"
  project       = "${google_project.vault-staging-gl.project_id}"
  force_destroy = false
  storage_class = "MULTI_REGIONAL"
...
data "google_iam_policy" "vault-backup-storage_binding" {
  binding {
    role = "roles/storage.legacyBucketOwner"
    members = "${local.legacyBucketOwner}"
  }
  binding {
    role = "roles/storage.legacyBucketReader"
    members = "${local.legacyBucketReader}"
  }
  binding {
    role = "roles/storage.admin"
    members = "${local.vault-backup-storage-admin}"
  }
  binding {
    role = "roles/storage.objectAdmin"
    members = "${local.vault-backup-object-admin}"
  }
}

resource "google_storage_bucket_iam_policy" "vault-backup-storage_policy" {
  bucket = "${google_storage_bucket.vault_backup.id}"
  policy_data = "${data.google_iam_policy.vault-backup-storage_binding.policy_data}"
}
*/
/*
# see PLATFORM-392 import fails
# terraform import google_storage_transfer_job.vault-backup-storage_daily vault-to-be-apped-1245-gl/7511847003481784272
*/
...
######################################################
#
# gl-vault helm chart inputs from terraform outputs
#
######################################################
/*
...
output "num_vault_pods" {
  value = "${var.num_vault_pods}"
}
output "vault_container" {
  value = "${var.vault_container}"
}
output "vault_init_container" {
  value = "${var.vault_init_container}"
}
output "vault_recovery_shares" {
...
# Uncomment this if you want to decrypt the root token yourself
output "root_token_decrypt_command" {
  value = "gsutil cat gs://${google_storage_bucket.vault.name}/root-token.enc | base64 --decode | gcloud kms decrypt --project ${google_project.vault-staging-gl.project_id} --location ${var.region} --keyring ${google_kms_key_ring.vault.name} --key ${google_kms_crypto_key.vault-init.name} --ciphertext-file - --plaintext-file -"
}
*/